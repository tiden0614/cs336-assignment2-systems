...
def _attention_and_lse(q, k, v, is_causal=False):
    ...
    # Transpose to (batch, head, seq, d_head) for attention computation
    ...
    d = q.shape[-1]
    scale = 1 / (d ** 0.5)
    S = einsum(q, k, 'b h q d, b h k d -> b h q k') * scale
    if is_causal:
        S = torch.where(
            torch.arange(n_queries, device=S.device)[None, None, :, None] >= torch.arange(n_keys, device=S.device)[None, None, None, :],
            S,
            -1e6
        )
    P = torch.softmax(S, dim=-1)
    o = einsum(P, v, 'b h q k, b h k d -> b h q d')
    L = torch.logsumexp(S, dim=-1)

    # Transpose back to (batch, seq, head, ...)
    o = rearrange(o, 'b h s d -> b s h d')
    L = rearrange(L, 'b h s -> b s h')
    return o, L
...
def _make_attn_inputs(device=None):
    ...
    num_heads = 4
    d_head = 64
    q = torch.randn(batch_size, n_queries, num_heads, d_head, device=device, requires_grad=True)
    k = torch.randn(batch_size, n_keys, num_heads, d_head, device=device, requires_grad=True)
    v = torch.randn(batch_size, n_keys, num_heads, d_head, device=device, requires_grad=True)
    do = torch.randn(batch_size, n_queries, num_heads, d_head, device=device)
...
def _test_flash_forward_pass(impl, device="cpu", is_causal=False):
    ...
    L_shape = (q.shape[0], q.shape[1], q.shape[2])  # (batch, seq, head)
    maybe_ls = [t for t in o.grad_fn.saved_tensors if t.shape == L_shape]

    assert len(maybe_ls) == 1, f"Expected one tensor of shape {L_shape} in saved tensors, but found {len(maybe_ls)}. The tests require you to save exactly one tensor of this shape, corresponding to the log-sum-exp of the attention scores."
    l = maybe_ls[0]
